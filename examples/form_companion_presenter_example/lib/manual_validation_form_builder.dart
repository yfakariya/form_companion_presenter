...
class _ManualValidationFormBuilderPane extends ConsumerWidget {
  @override
...
@visibleForTesting
class ManualValidationFormBuilderPresenter
    extends FormBuilderPresenter<TargetState> {
  ManualValidationFormBuilderPresenter()
      : super(
  ...
  FutureOr<void> doSubmit(BuildContext context) async {
    if (!(await validateAll(maybeFormStateOf(context)!))) {
      return;
    }
...
final _presenter =
    StateNotifierProvider<ManualValidationFormBuilderPresenter, TargetState>(
  (_) => ManualValidationFormBuilderPresenter(),
);
