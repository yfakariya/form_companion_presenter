...
try {
    ...
    if (ShouldGenerateLcovInfo $inputPath $lcovFilePath) {
        ...
        fvm dart pub global run coverage:format_coverage --packages=.packages --check-ignore --report-on=lib --lcov -i "$inputPath" -o $lcovFilePath --packages="$pacakageJsonPath"
