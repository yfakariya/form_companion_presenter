  ...
  void visitPrefixedIdentifier(PrefixedIdentifier node) {
    if (node.staticType != null) {
      _logger.finer(
        ...
        'type is ${node.staticType}(${node.staticType?.element}).',
      );
...
@sealed
class DependentLibraryCollector2 {
  static final _dartCoreLibraryId = 'dart:core';
  ...
  /// Initializes a new [DependentLibraryCollector2] instance.
  ///
  /// [presenterLibrary] is the library itself to be scanned.
  /// References to this library will not be collected.
  DependentLibraryCollector2(
    this._nodeProvider,
    this._allLibraries,
    this._logger,
    LibraryElement presenterLibrary,
  ...
  FutureOr<AstNode> _getFieldElementDeclarationAsync(
    AstNode node,
    String fieldName,
  ) async {
    InterfaceElement? targetClass = _contextClass;
    ...
    throw AnalysisException(
      "Failed to get correspond field '$fieldName' for parameter "
      'in $_contextClass class hierarchy at ${getNodeLocation(node, _contextClass)}.',
    );
  }
  ...
  FutureOr<void> _handleParameterAsync(FormalParameter node) async {
    if (node is FieldFormalParameter) {
      await _handleFormalParameterAsync(node, node.type);
    } else if (node is SuperFormalParameter) {
      await _handleFormalParameterAsync(node, node.type);
    } else if (node is DefaultFormalParameter) {
      await _handleParameterAsync(node.parameter);
      final defaultValue = node.defaultValue;
      if (defaultValue != null) {
        _handleExpression(node.declaredElement, defaultValue);
      }
    } else {
      throw AnalysisException(
        'Unexpected parameter type: ${node.runtimeType} '
        'at ${getNodeLocation(node, _contextClass)}',
      );
    }
  }

  FutureOr<void> _handleFormalParameterAsync(
    ...
    if (type == null) {
      ...
      final field = await _getFieldElementDeclarationAsync(node, name.lexeme);
