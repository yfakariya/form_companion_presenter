...
* [State restoration](https://api.flutter.dev/flutter/services/RestorationManager-class.html) even if you use `DropdownButtonFormField`.
...
8. Create a widget. We use `ConsumerWidget` here. Note that you must place `Form` and `FormFields` to separate widget. Note that `state.fields` have form field factories generated by `form_companion_generator` and their type can be controlled in the presenter:
...
See [repository](https://github.com/yfakariya/form_companion_presenter/tree/main/examples/form_companion_presenter_example). `*_vanilla_form_*.dart` files use `form_companion_presenter`. Note that `*_form_builder_*.dart` files use [form_builder_companion_presenter](https://pub.dev/packages/form_builder_companion_presenter) instead.
...
* `FormPropertiesRestorationScope` which enables [State restoration](https://api.flutter.dev/flutter/services/RestorationManager-class.html).
